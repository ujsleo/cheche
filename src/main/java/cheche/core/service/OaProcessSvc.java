...
public class OaProcessSvc {
	...
	public Long apply(ApplyContent applyContent) {
		...
		if (processPojo != null) { // 再次发起
			...
			// 创建审批流程实例
			ChecheApplyProcess sample = applyContent.toProcessPojo();
			processDao.insertSelective(sample);
			processId = sample.getId();
		}

		// 添加新申请数据
		controlDao.batchInsert(applyContent.toControlPojo(processId));
	...
	private void check4Withdraw(WithdrawOpt withdrawOpt) {
		...
		if (processPojo == null)
			throw new ChecheException(ChecheExceptionEnum.E400, //
					String.format("Illegal opt, process_id: %l NOT exists.", processId));
		// 校验用户是否可以撤回
		if (!ApplyProcessStatus.canWithdraw(processPojo.getStatus()))
			throw new ChecheException(ChecheExceptionEnum.E403, //
					String.format("Illegal opt, process_id: %l, status: %d can NOT withdraw.", processId,
							processPojo.getStatus()));
		if (!withdrawOpt.getUser().equals(processPojo.getUser()))
			throw new ChecheException(ChecheExceptionEnum.E403, //
					String.format("Illegal opt, process_id: %l, user: %s can NOT withdraw.", processId,
							withdrawOpt.getUser()));
	}
	...
	public void migration(Long processId, Integer step) {
		...
		if (step > templateContent.totalSteps()) {
			...
			doMigration(processId, approver, step);
			processDao.updateStep(processId, step);
		}
	}
	...
	private void doMigration(Long processId, ApproverItem approver, Integer step) {
		// 回调业务系统发起
		OaTaskHelper.start(approver.getClassName());
