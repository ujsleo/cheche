...
public class OaTaskSvc {
    ...
    public void pass(ApproveOpt approveOpt) {
        ...
        Integer step = taskPojo.getStep();
        switch (taskPojo.getType()) {
            case ApplyTaskType.OR:
                // 回调业务系统通过
                OaTaskHelper.pass(taskPojo.getClassName(), new OaVariables(processId, taskId, step));

                // task PASS
                taskDao.pass(taskId, approveOpt.getUser());
        ...
        processSvc.migration(processId, nextStep, approveOpt.getUser());
    ...
    private void check4Opt(ApproveOpt approveOpt) {
        ...
        if (taskPojo == null)
            throw new ChecheException(ChecheExceptionEnum.E400,
                    String.format("Illegal opt, task_id: %d NOT exists.", taskId));
        // 校验用户是否可以审批
        if (!ApplyTaskStatus.canOpt(taskPojo.getStatus()))
            throw new ChecheException(ChecheExceptionEnum.E403,
                    String.format("Illegal opt, task_id: %d, status: %d can NOT opt.", taskId, taskPojo.getStatus()));
        ...
        if (!users.contains(approveOpt.getUser()))
            throw new ChecheException(ChecheExceptionEnum.E403,
                    String.format("Illegal opt, task_id: %d, user: %s can NOT opt.", taskId, approveOpt.getUser()));
    }
    ...
    public void reject(RejectOpt rejectOpt) {
        ...
        // 流程终止
        processDao.terminated(processId, targetStep, rejectOpt.getUser());
